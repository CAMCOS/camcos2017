...
mainfunction <- function(data, nclust, sparse,
    weightfunction="IDF", simfunction = "cosine",clusterfunction="DiffusionMap",
    
    # Column weighting argument defaults:
    par1=NULL, par2=NULL, mode=NULL, binary=TRUE, convertsparse=TRUE,lower=2, upper=NULL,
    
    # Similarity function argument defaults:
    rowscaling = NULL, colscaling = NULL, sigma = NULL, centers = NULL, seed = NULL, 
    distance = NULL, simscale=NULL,
    
    # Clustering function argument defaults:
    t=.5, kmeans.method="kmeans", m=NULL,
    
    # SVD options for weighted (IDF) data:
    weight.SVD=FALSE, SVDdim=100, SVDprint=FALSE,
    ...
    SVDsim=FALSE, simdim=3, dim1sim=1, dim2sim=2, dim3sim=3, sim.filepath=NULL,
    
    # Cluster Insights:
    insights=FALSE, n.ins=NULL, vocab=NULL, nfeatures=20, insight.plot = TRUE, insight.filepath = NULL,
    ...
    if (insights==TRUE) {
        ...
        for (clustID in 1:n.ins) {
            ...
            getInsights(cluster=weighteddata[clusterdata$clus==clustID,], vocab=vocab, 
